...
def test_adj_motion_classifier(classifier_dict, adjective_name, test_feature_objs, feature_list):
    '''
    ...
    for motion in test_feature_objs:
        ...
        for test_obj in motion_test_obj_list:
            if test_obj.labels == None:
                prediction, probability = utilities.compute_adjective_probability_score(classifier_dict, test_obj, feature_list, adjective_name)
            else:
                prediction, probability, truth = utilities.compute_adjective_probability_score(classifier_dict, test_obj, feature_list, adjective_name)
        ...
        if len(results) > 1: 
            ...
            # Store the scores
            object_run_scores[motion] = (f1_score(truth_vector, results))
...
# MAIN FUNCTION
def main(classifiers_pkl, all_classifiers_pkl, test_feature_pkl, ensemble_test_feature_pkl):
    ...
    if all_classifiers_pkl == None:
        ...
        test_adj_motion_classifier(classifiers_dict, adjective_name, ensemble_feature_dict, feature_name_list) 
    else:
        ...
        for adj in classifiers_dict:
           ...
           probility_vector, motion_scores = test_adj_motion_classifier(classifiers_dict[adj], adj, ensemble_feature_dict, feature_name_list)
...
def parse_arguments():
    ...
    return classifiers_pkl, all_classifiers_pkl, test_feature_pkl, ensemble_test_feature_pkl

if __name__ == "__main__":
    classifiers_pkl, all_classifiers_pkl, test_feature_pkl, ensemble_test_feature_pkl = parse_arguments()
    main(classifiers_pkl, all_classifiers_pkl, test_feature_pkl, ensemble_test_feature_pkl)
