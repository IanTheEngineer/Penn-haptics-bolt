...
def dict_from_h5_group(group, alt_phases = None, alt_sensors = None):
    ...
    for phase in all_phases:
        ...
        for sensor in all_sensors:
            ...
            if sensor == "gripper_aperture":
                gripper = group.gripper_aperture
                gripper_pos = getattr(gripper, 'joint_position').read()
                nrows = gripper_pos.shape[0]
                gripper_pos = gripper_pos.reshape((nrows,-1))
                gripper_pos = gripper_pos[indexed,:]      
                
                phase_data[sensor] = gripper_pos 
           
            elif sensor == "transforms":
                transform = group.transforms
                translation = getattr(transform, 'translation').read()
                nrows = translation.shape[0]
                translation = translation.reshape((nrows,-1))
                translation = translation[indexed,:]
            
                phase_data[sensor] = translation 
            else: # Biotacs
                #finger 0
